    ...
    <div class="md:flex-col md:items-stretch md:min-h-full md:flex-nowrap px-0 flex flex-wrap items-center justify-between w-full mx-auto">
        ...
        <div class="md:flex md:flex-col md:items-stretch md:opacity-100 md:relative md:mt-4 md:shadow-none shadow absolute top-0 left-0 right-0 z-40 overflow-y-auto overflow-x-hidden h-auto items-center flex-1 rounded hidden" id="example-collapse-sidebar">
            ...
            <ul class="md:flex-col md:min-w-full flex flex-col list-none">
                ...
                @endcan
                @can('order_request_access')
                    <li class="items-center">
                        <a class="{{ request()->is("admin/order-requests*") ? "sidebar-nav-active" : "sidebar-nav" }}" href="{{ route("admin.order-requests.index") }}">
                            <i class="fa-fw c-sidebar-nav-icon fas fa-cart-plus">
                            </i>
                            {{ trans('cruds.orderRequest.title') }}
                        </a>
                    </li>
                @endcan
                @can('order_transfer_access')
                    <li class="items-center">
                        <a class="{{ request()->is("admin/order-transfers*") ? "sidebar-nav-active" : "sidebar-nav" }}" href="{{ route("admin.order-transfers.index") }}">
                            <i class="fa-fw c-sidebar-nav-icon fas fa-exchange-alt">
                            </i>
                            {{ trans('cruds.orderTransfer.title') }}
                        </a>
                    </li>
                @endcan
